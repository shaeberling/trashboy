...
static int cassette_sample_rate = SDM_SAMPLING_FREQ;
int cassette_default_sample_rate = SDM_SAMPLING_FREQ;
...
static inline void refresh_cassette_sample_rate()
{
  int rate = sdm_get_effective_sample_rate();
  if (rate > 0) {
    cassette_sample_rate = rate;
    cassette_default_sample_rate = rate;
  }
}
...
transition_out(int value, tstate_t z80_state_t_count)
{
  ...
  ddelta_us = (z80_state_t_count - cassette_transition) / CLOCK_MHZ_M3
    ...
    while (nsamples-- > 0 && trsSamplesGenerator != NULL) {
      if (!trsSamplesGenerator->putSample(sample)) {
        break;
      }
    }
    #if 0
