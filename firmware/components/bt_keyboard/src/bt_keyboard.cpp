...
// USB HID Keyboard code to ASCII/Special character mapping (US keyboard layout)
// Format: Each HID keycode (starting from 4) maps to TWO characters: [normal, shifted]
// 
// Keycode layout (HID Usage Page 0x07 - Keyboard/Keypad):
//   4-29:   a-z letters (pairs: lowercase, uppercase)
//  30-39:   1-0 numbers (pairs: number, shifted symbol: !@#$%^&*())
...
void BTKeyboard::remove_all_bonded_devices() {
  ...
  if (dev_count > 0) {
    ...
    ESP_LOGD(TAG, "Removed %d BLE bonded devices.", dev_count);
  } else {
    ESP_LOGD(TAG, "No BLE bonded devices to remove.");
  }

#if CONFIG_BT_HID_HOST_ENABLED
  // Remove Classic Bluetooth bonded devices
  int bt_dev_count = esp_bt_gap_get_bond_device_num();
  
  if (bt_dev_count > 0) {
    esp_bd_addr_t *bt_dev_list = new esp_bd_addr_t[bt_dev_count];
    
    if (esp_bt_gap_get_bond_device_list(&bt_dev_count, bt_dev_list) == ESP_OK) {
      for (int i = 0; i < bt_dev_count; i++) {
        esp_bt_gap_remove_bond_device(bt_dev_list[i]);
      }
      ESP_LOGD(TAG, "Removed %d Classic Bluetooth bonded devices.", bt_dev_count);
    } else {
      ESP_LOGE(TAG, "Failed to get Classic Bluetooth bonded device list.");
    }
    
    delete[] bt_dev_list;
  } else {
    ESP_LOGD(TAG, "No Classic Bluetooth bonded devices to remove.");
  }
#endif // CONFIG_BT_HID_HOST_ENABLED
}
