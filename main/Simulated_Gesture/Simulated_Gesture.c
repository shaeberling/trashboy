...
void Add_Style_Label(lv_obj_t *Label_obj, lv_style_t * Label_style){
  if(Label_obj){
    ...
    lv_area_t coords;                                 
    lv_obj_get_coords(Label_obj, &coords);
    if (coords.y2 > EXAMPLE_LCD_V_RES*7/10) {   
      scroll_up_fixed(Page_panel[current_Page_sequence],coords.y2 - (EXAMPLE_LCD_V_RES*7/10));
    }
    else if (coords.y1 < EXAMPLE_LCD_V_RES*3/10) {   
      scroll_up_fixed(Page_panel[current_Page_sequence],coords.y1 - (EXAMPLE_LCD_V_RES*3/10));
    }
